...
func (pasteBin *PasteBinInfo) TrendingPastes() (*Pastes, error) {
    query := url.Values{}
    query.Add("api_dev_key", pasteBin.APIKey)
...
// Paste options, made up of an access level, expiration time, format, and
// title.  Format is one of: http://pastebin.com/api#5
...
// set to 'Public', Format is set to 'text', and Title is 'Untitled'.
func DefaultOptions() *PasteOptions {
    return &PasteOptions{Public, Never, "text", "Untitled"}
}

// Make an anonymous paste. An anonymous paste can only be Public or Unlisted.
func (pasteBin *PasteBinInfo) AnonymousPaste(content *string, options *PasteOptions) (*url.URL, error) {
    query := url.Values{}
    query.Add("api_dev_key", pasteBin.APIKey)
    ...
    query.Add("api_paste_name", options.Title)
    ...
    if err != nil {
        return nil, &PasteError{"invalid paste url: " + err.Error()}
    }
...
func (pasteBin *PasteBinInfo) UserPaste(content *string, options *PasteOptions) (*url.URL, error) {
    query := url.Values{}
    query.Add("api_dev_key", pasteBin.APIKey)
    query.Add("api_option", "paste")
    query.Add("api_user_key", pasteBin.UserKey)
